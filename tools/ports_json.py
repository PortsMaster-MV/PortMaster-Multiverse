...
PORT_SOURCE = {
    "prefix": REPO_CONFIG['REPO_PREFIX'],
    "api": "PortMasterV2",
    "name": REPO_CONFIG['REPO_NAME'],
    "url": f"https://api.github.com/repos/{REPO_CONFIG['RELEASE_ORG']}/{REPO_CONFIG['RELEASE_REPO']}/releases/latest",
    "last_checked": None,
    "version": 4,
    "data": {},
    }
...
def check_port(port_name, zip_file, extra_data=None):
    ...
    with zipfile.ZipFile(zip_file, 'r') as zf:
            ...
            if '/' in file_info.filename:
                ...
                if len(parts) == 2:
                    if parts[1].lower().endswith('.port.json'):
                        ...
                        port_info_files.append(file_info.filename)
        ...
        if len(port_info_files) > 1:
            extra_data['warnings'].append(f"Port {port_name} has multiple port.json files.")
            for pif in port_info_files:
                extra_data['warnings'].append(f"- {pif}")
...
def main(args):
    ...
    source_json = Path('.') / f"010_{name_cleaner(REPO_CONFIG['REPO_NAME'])}.source.json"
    ...
    for port_name in sorted(all_ports, key=lambda port_name: port_name.lower()):
        ...
        elif len(extra_info[port_name]['warnings']) > 0:
            print(f"::warning file={port_name}::Bad port {port_name}")

        if len(extra_info[port_name]['errors']):
            for message in extra_info[port_name]['errors']:
                print(f"::error file={port_name}::{message}")

            errors = True

        if len(extra_info[port_name]['warnings']):
            for message in extra_info[port_name]['warnings']:
                print(f"::error file={port_name}::{message}")
    ...
    with source_json.open('w') as fh:
        json.dump(PORT_SOURCE, fh, indent=4)
